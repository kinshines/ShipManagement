...
namespace Ship.Web.Controllers
{
    public class ContractController : Controller
    {
        ...
        readonly ServiceRecordService _serviceRecordService;
        OperationResult operationResult;
        readonly IHostingEnvironment _env;
        public ContractController(ContractService contractService, SailorService sailorService, VesselService vesselService,
            ShipownerService shipownerService, ServiceRecordService serviceRecordService, NoticeService noticeService,
        ...
        {
            string SailorID = Request.Query["SailorID"];
            string VesselID = Request.Query["VesselID"];
        ...
        private FileResult ExportContract(Dictionary<String, String> contractDic, string companyId, string sailorId, string sailorName)
        {
            ...
            string sourcepath = _env.ContentRootFileProvider.GetFileInfo("/Files/Contract/Template/" + companyId + ".docx").PhysicalPath;
            if (!System.IO.File.Exists(sourcepath))
                sourcepath = _env.ContentRootFileProvider.GetFileInfo("/Files/Contract/Template/0.docx").PhysicalPath;
            string targetPath = _env.ContentRootFileProvider.GetFileInfo("/Files/Contract/Export/" + sailorId + "_外派合同.docx").PhysicalPath;
            ...
            var name = Path.GetExtension(targetPath);
            var provider = new FileExtensionContentTypeProvider();
            string contentType = provider.Mappings[name];
            return File(targetPath, contentType, WebUtility.UrlEncode(sailorName + "_外派合同.docx"));
        }
    }
}