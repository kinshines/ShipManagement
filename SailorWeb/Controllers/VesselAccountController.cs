...
namespace SailorWeb.Controllers
{
    ...
    {
        readonly IVesselAccountService _accountService;
        ...
        public VesselAccountController(IVesselAccountService accountService,IVesselService vesselService,
            ICompanyService companyService, IShipownerService shipownerService, IUploadFileService uploadService)
        {
        ...
        public ActionResult Cost(VesselAccount vesselcost)
        {
            if (ModelState.IsValid)
            {
                HttpPostedFileBase file = Request.Files["invoiceFile"];
                vesselcost.InvoiceFileID = _uploadService.AddFile(file);
                ...
                if (payment.Payment > 0 || payment.USPayment > 0)
                {
                    ...
                    payment.ReceiptFileID = _uploadService.AddFile(file);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Settle(SettlePayment settle)
        {
            if (ModelState.IsValid)
            {
                ...
                HttpPostedFileBase file = Request.Files["receiptFile"];
                payment.ReceiptFileID = _uploadService.AddFile(file);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Deposit([Bind(Include = "VesselDepositID,Deposit,USDeposit,Remark,PaymentDate,VesselID,ShipownerID")] VesselAccount vesseldeposit)
        {
            if (ModelState.IsValid)
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult SignFile(int? VesselAccountID) 
        {
            var cost = _accountService.Find(VesselAccountID);
            HttpPostedFileBase file = Request.Files["signFile"];
            cost.SignFileID = _uploadService.AddFile(file);
